...
int main(int argc, char *argv[]){
	...
	int charlength=400, i, j, k, chemistrymax=20, sidechaintypesmax=10, chaintypes, *Nchainsoftype, *chainlengthoftype, **chainchemistry, Nchains, Nmonomers, Nnodes, *chainlength, Nnodetypes, movieatoms, allatoms, cgatoms, cycles, t, movetype, moviecycle, calccycle, calcsperoutput, bestframe=0, bestallatomframe=0, chargetype, lastchargetype, selftype, leaf, mctype, leafsymmetry, terminuscode, fixedinterchainspacing, biasaminotrans;
	double kelvins, saltmolarity, temperature, oldenergy, newenergy, acceptanceprob, lowestenergy, randval;
	char **chemistry_char;
	chemistry_char=xmalloc(sizeof(char *)*chemistrymax);
	for(i=0;i<chemistrymax;i++) chemistry_char[i]=xmalloc(sizeof(char)*charlength);
	linkedlistset linkset;
	double_triple box_dimension, bestavboxdimension, shift;
	coord *coordarray;
	...
	monomercount.type=malloc(sizeof(double)*sidechaintypesmax+1);
	...
	my_bonded_params.sr=-4.10588;
 	my_bonded_params.sidechain=xmalloc(sizeof(sidechain_params)*sidechaintypesmax+1);
	...
	my_bonded_params.sidechain[3].r32=-0.210233;
 	my_bonded_params.sidechainorientationtype=xmalloc(sizeof(int)*sidechaintypesmax+1);
	...
	mctype=loadparam(argc, "-mctype", argv, "0").i;
	strcpy(inputfile, loadparam(argc, "-inputfile", argv, "0").s);
	strcpy(base, loadparam(argc, "-base", argv, "../bilayer").s);
	...
	my_energycomponents.solvation=malloc(sizeof(double)*Nnodetypes+1);
	my_energycomponents.interface=malloc(sizeof(double)*Nnodetypes+1);
	bestmy_energycomponents.solvation=malloc(sizeof(double)*Nnodetypes+1);
	bestmy_energycomponents.interface=malloc(sizeof(double)*Nnodetypes+1);

	//	Configuration parameters
	
	bilayer config, delta, newconfig;
	...
	allocate_3d_tensor_nozero(double_triple, delta.backboneoffset, 2, Nnodetypes, Nnodetypes);
    allocate_3d_tensor(double, delta.backbonenz, 2, Nnodetypes, Nnodetypes);
    allocate_3d_tensor(double, delta.backbonephi, 2, Nnodetypes, Nnodetypes);
    allocate_3d_tensor_nozero(double_triple, delta.sidechainrelativepos, 2, Nnodetypes, Nnodetypes);
    allocate_3d_tensor(double, delta.sidechainnpar, 2, Nnodetypes, Nnodetypes);
    allocate_3d_tensor(double, delta.sidechainnphi, 2, Nnodetypes, Nnodetypes);
    ...
    for(k=0;k<2;k++){
        for(i=0;i<Nnodetypes;i++){
            for(j=0;j<Nnodetypes;j++){
                delta.backboneoffset[k][i][j].x=delta.backboneoffset[k][i][j].y=delta.backboneoffset[k][i][j].z=0.1;
                delta.backbonenz[k][i][j]=0.05;
                delta.backbonephi[k][i][j]=0.1;
                delta.sidechainrelativepos[k][i][j].x=delta.sidechainrelativepos[k][i][j].y=delta.sidechainrelativepos[k][i][j].z=0.1;
                delta.sidechainnpar[k][i][j]=0.05;
                delta.sidechainnphi[k][i][j]=0.1;
            }
        }
    }
	...
	initialize_periodic_bilayer(Nmonomers, Nchains, &box_dimension, chainlength, monomerid, coordarray, my_bonded_params, my_nonbonded_params, config, leafsymmetry);
	...
	sprintf(bestallatommoviefile, "%s/c%i.allatom.best.xyz", base, cycles);
	sprintf(bestallatomsourcefile, "%s/c%i.allatom.best.vmd.source", base, cycles);
	...
	calc_cellstruct_leafid(coordarray, my_bonded_params, my_nonbonded_params, Nchains, chainlength, monomerid, box_dimension, &bestmy_energycomponents, linkset, bestavheight, &bestavboxdimension);
	output_timeseries(besttotalenergyfile, &bestmy_energycomponents, 0, monomercount, 1., Nnodetypes, bestavheight, &bestavboxdimension, 0);
    output_bilayer(bestconfigfile, config, oldenergy, 0);
	...
	for(t=0;t<cycles;){
		...
		if(mctype!=1){
			if(movetype==0) newconfig.monomerspacing+=(2*rand_double-1)*delta.monomerspacing;
			if(movetype==1) newconfig.monomerscaleoffsetfraction+=(2*rand_double-1)*delta.monomerscaleoffsetfraction;
			if(movetype==2) newconfig.terminusspacing+=(2*rand_double-1)*delta.terminusspacing;
			if(movetype<3){
			...
			if(movetype==3) newconfig.interchainspacing+=(2*rand_double-1)*delta.interchainspacing;
			if(movetype==4) newconfig.backboneoffset[leaf][lastchargetype][selftype].x+=(2*rand_double-1)*delta.backboneoffset[leaf][lastchargetype][selftype].x;
			if(movetype==5) newconfig.backboneoffset[leaf][lastchargetype][selftype].y+=(2*rand_double-1)*delta.backboneoffset[leaf][lastchargetype][selftype].y;
			if(movetype==6) newconfig.backboneoffset[leaf][lastchargetype][selftype].z+=(2*rand_double-1)*delta.backboneoffset[leaf][lastchargetype][selftype].z;
			if(movetype==7){
				newconfig.backbonenz[leaf][lastchargetype][selftype]+=(2*rand_double-1)*delta.backbonenz[leaf][lastchargetype][selftype];
				reflect(&(newconfig.backbonenz[leaf][lastchargetype][selftype]));
			}
			if(movetype==8){
				newconfig.backbonephi[leaf][lastchargetype][selftype]+=(2*rand_double-1)*delta.backbonephi[leaf][lastchargetype][selftype];
				fmod((newconfig.backbonephi[leaf][lastchargetype][selftype]), (2*M_PI));
			}
			if(movetype==9) newconfig.sidechainrelativepos[leaf][lastchargetype][selftype].x+=(2*rand_double-1)*delta.sidechainrelativepos[leaf][lastchargetype][selftype].x;
			if(movetype==10) newconfig.sidechainrelativepos[leaf][lastchargetype][selftype].y+=(2*rand_double-1)*delta.sidechainrelativepos[leaf][lastchargetype][selftype].y;
			if(movetype==11) newconfig.sidechainrelativepos[leaf][lastchargetype][selftype].z+=(2*rand_double-1)*delta.sidechainrelativepos[leaf][lastchargetype][selftype].z;
			if(movetype==12){
				newconfig.sidechainnpar[leaf][lastchargetype][selftype]+=(2*rand_double-1)*delta.sidechainnpar[leaf][lastchargetype][selftype];
				reflect(&(newconfig.sidechainnpar[leaf][lastchargetype][selftype]));
			}
			if(movetype==13){
				newconfig.sidechainnphi[leaf][lastchargetype][selftype]+=(2*rand_double-1)*delta.sidechainnphi[leaf][lastchargetype][selftype];
				fmod((newconfig.sidechainnphi[leaf][lastchargetype][selftype]), (2*M_PI));
			}
			if(movetype==14){
				randval=rand_double;
				for(i=0;i<Nnodetypes;i++){
					for(j=0;j<Nnodetypes;j++){
						newconfig.backboneoffset[leaf][i][j].x+=(2*randval-1)*delta.backboneoffset[leaf][i][j].x;
					}
				}
			}
			if(movetype==15){
				...
				for(i=0;i<Nnodetypes;i++){
					for(j=0;j<Nnodetypes;j++){
						newconfig.backboneoffset[leaf][i][j].y+=(2*randval-1)*delta.backboneoffset[leaf][i][j].y;
					}
				}
			}
			if(movetype==16){
				...
				for(i=0;i<Nnodetypes;i++){
					for(j=0;j<Nnodetypes;j++){
						newconfig.backboneoffset[leaf][i][j].z+=(2*randval-1)*delta.backboneoffset[leaf][i][j].z;
					}
				}
			}
		}
		...
		if((acceptanceprob>1)||(rand_double<acceptanceprob)){
			copybilayer(newconfig, &config, Nnodetypes);
			oldenergy=newenergy;
            if(oldenergy<lowestenergy){
                ...
                calc_cellstruct_leafid(coordarray, my_bonded_params, my_nonbonded_params, Nchains, chainlength, monomerid, box_dimension, &bestmy_energycomponents, linkset, bestavheight, &bestavboxdimension);
				output_timeseries(besttotalenergyfile, &bestmy_energycomponents, t, monomercount, 1., Nnodetypes, bestavheight, &bestavboxdimension, 0);
                output_bilayer(bestconfigfile, config, oldenergy, t+1);
                lowestenergy=oldenergy;
            }
		}
		t++;
